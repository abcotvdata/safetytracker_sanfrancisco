...
<br>
<h2>ABC7 is tracking crime and safety across the city of San Francisco and in your neighborhood. 
...
<bignumber>`r prettyNum(round(murders_city$avg_prior4years,0), big.mark=",")`<br></bignumber><p>
Yearly average<br>2020 to 2023</p>
</div>
<div class="card" style="background-color:#99a0a5;">
<p>Homicide Rate<br>
...
<bignumber>`r murders_city$rate_prior4years`<br></bignumber><p>
Per 100,000 people<br>2020 to 2023</p>
</div>
</div>
<br>
Homicides over the last 12 months are trending <h8>`r case_when(murders_city$inc_23tolast12 >= .25 ~ " up", murders_city$inc_23tolast12<=-.25 ~ " down", TRUE ~ " about the same")` `r ifelse(murders_city$inc_23tolast12>=0,murders_city$inc_23tolast12,substring(murders_city$murders_city$inc_23tolast12,2))`%</h8> when compared to last year, according to San Francisco Police Department data through `r format(asofdate, "%B %d")`.

The murder rate over the last 12 months is <h8>`r case_when((murders_city$rate_last12-murders_city$rate_prior4years)>=0.25 ~ "up",
          (murders_city$rate_last12-murders_city$rate_prior4years)<=-0.25 ~ "down",
          TRUE ~ "about the same ")` `r ifelse(murders_city$inc_prior4yearavgtolast12>=0,murders_city$inc_prior4yearavgtolast12,substring(murders_city$inc_prior4yearavgtolast12,2))`%</h8> compared to the annual average over the last four years.

The city averaged about <h8>`r round(murders_city$last12mos/12,0)`</h8> homicides a month over the last 12 months. In 2020, that number was about <h8>`r round(murders_city$total20/12,0)`</h8> a month.

One way to think about the danger: three years ago, the murder rate was <h8>`r murders_city$rate20`</h8> per 100,000 residents. That's lower than the risk of dying in a vehicle crash in California. The rate has risen slightly to <h8>`r murders_city$rate_last12`</h8> in the last 12 months.
<br>
<br>
<!-- <subhead>Risk of being murdered in San Francisco vs. other causes of death in Harris County</subhead> -->
...
ABC7's data team looked at the San Francisco Police Department's crime incident data by police district from 2019 through `r format(asofdate, "%B %d, %Y")`. Totals here include murders, but not involuntary manslaughter or justifiable homicide.

<subhead>A closer look at San Francisco homicides by neighborhood</subhead>
...
"</td>
				...
				<td>",
murders_district$avg_prior4years,
"</td>
				<td>",
murders_district$rate_prior4years,
"</td>
			</tr>
</table>")
...
san_francisco_murder_map <- leaflet(murders_district, options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.5, zoomDelta=0.5)) %>%
...
<br>
<br>
<iframe title="Homicide clearance rate year by year in San Francisco" aria-label="Column Chart" id="datawrapper-chart-eYqDD" src="https://datawrapper.dwcdn.net/eYqDD/1/" scrolling="no" frameborder="0" style="width: 0; min-width: 100% !important; border: none;" height="538" data-external="1"></iframe><script type="text/javascript">!function(){"use strict";window.addEventListener("message",(function(e){if(void 0!==e.data["datawrapper-height"]){var t=document.querySelectorAll("iframe");for(var a in e.data["datawrapper-height"])for(var r=0;r<t.length;r++){if(t[r].contentWindow===e.source)t[r].style.height=e.data["datawrapper-height"][a]+"px"}}}))}();
</script>
<br>
<br>
